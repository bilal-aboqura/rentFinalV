...
export default function AdminContentManager({ initialSettings }: AdminContentManagerProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    brand_primary_color: colorInputValue(initialSettings.brand_primary_color, '#800000'),
    brand_secondary_color: colorInputValue(initialSettings.brand_secondary_color, '#0b0c10'),
  });
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, assetType: 'logo' | 'hero') => {
    ...
    try {
      ...
      if (res.success && res.url) {
        ...
        setSettings((prev) => ({
          ...prev,
          site_logo_url: assetType === 'logo' ? res.url ?? prev.site_logo_url : prev.site_logo_url,
          hero_image_url: assetType === 'hero' ? res.url ?? prev.hero_image_url : prev.hero_image_url,
          updated_at: new Date().toISOString(),
        }));
      } else {
