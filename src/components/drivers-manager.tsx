...
const STATUS_STYLES: Record<string, string> = {
  ...
  inactive: 'bg-slate-100 text-slate-500 border-slate-600',
};
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        {/* Search */}
        <form onSubmit={handleSearch} className="flex w-full flex-col gap-2 sm:max-w-sm sm:flex-1 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-white/60 border border-slate-700 text-slate-900 rounded-xl pl-9 pr-8 py-2 text-sm focus:outline-none focus:border-indigo-500 transition-all placeholder:text-slate-600"
            />
            {searchInput && (
              <button
                type="button"
          ...
          <button
            ...
            type="submit"
            className="rounded-xl bg-slate-100 px-3 py-2 text-sm text-slate-900 transition-all hover:bg-slate-600"
          >
            Search
          </button>
        </form>
              ...
              <article key={driver.id} className="mobile-data-card">
                ...
                <div className="space-y-3">
                  <div className="mobile-data-row">
                    <span className="mobile-data-label">Plate</span>
                    <span className="mobile-data-value font-mono" dir="ltr">
                      {driver.license_plate}
                    </span>
                  </div>
                  <div className="mobile-data-row">
                    <span className="mobile-data-label">Since</span>
                ...
                {deleteConfirmId === driver.id ? (
                  <div className="mt-4 grid gap-2">
                    <p className="text-xs font-medium text-red-500">Confirm delete?</p>
                    <div className="flex gap-2">
                      <button
                        id={`confirm-delete-driver-mobile-${driver.id}`}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {drivers.map((driver) => (
                  ...
                  >
                    <td className="px-5 py-4 text-slate-900 font-medium">{driver.name}</td>
                    <td className="px-5 py-4 text-slate-700 font-mono text-xs">{driver.phone}</td>
                    <td className="px-5 py-4 text-slate-700 font-mono text-xs">{driver.license_plate}</td>
                      ...
                      <div className="flex items-center gap-2 justify-end">
                        ...
                        {deleteConfirmId === driver.id ? (
                          <div className="flex items-center gap-1.5">
                            ...
                            <button
                              onClick={handleDeleteCancel}
                              className="text-xs text-slate-500 hover:text-slate-900 px-2 py-1 rounded hover:bg-black/5 transition-all"
                            >
                              Cancel
                            </button>
                          </div>
        ...
        <div className="flex flex-col gap-3 text-sm sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={page <= 1}
              className="p-1.5 rounded-lg text-slate-500 hover:text-slate-900 hover:bg-black/5 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
            >
